...
if (-not $Unregister) {
    Write-Host "DEPRECATED: harmonogram DE GU wylaczony z produkcji." -ForegroundColor Yellow
    Write-Host "Zainstaluj kampanie UA:" -ForegroundColor Cyan
    Write-Host "  powershell -ExecutionPolicy Bypass -File schedule\ua\register_tasks_5_dni.ps1"
    Write-Host "Wyrejestruj stare zadania DE:" -ForegroundColor Yellow
    Write-Host "  powershell -ExecutionPolicy Bypass -File schedule\register_tasks_5_dni.ps1 -Unregister"
    exit 1
}
