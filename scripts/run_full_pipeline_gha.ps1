...
<#
Uruchamia pelny pipeline UA na GitHub Actions (recznie, krok po kroku).
...
$Repo = "Bigmax1993/wyszukiwarka-materialow-budowlanych-ukraina"
$DiscoveryWorkflow = "UA discovery"
$DiscoveryArtifacts = @("ua-materialy-wyniki-pi")
...
if (-not $SkipBackfill) {
    Invoke-GhaWorkflow "UA niedziela backfill"
}

Invoke-GhaWorkflow "Sync wyniki Google Drive UA" @{ artifact_name = "ua-materialy-wyniki-thu" }
Invoke-GhaWorkflow "UA poniedzialek prep"
Invoke-GhaWorkflow "UA poniedzialek send" $sendFields
Invoke-GhaWorkflow "UA wtorek send" $sendFields

Write-Host "`nPipeline UA zakonczony pomyslnie." -ForegroundColor Green
