...
def first_email_from_contact(info: dict) -> str:
    ...
    if email and "@" in email:
        return email
    for key in ("emails_found", "impressum_emails_found"):
        found = [
            x.strip()
            for x in _s(info.get(key)).split(",")
            if x.strip() and "@" in x
        ]
        if found:
            return found[0]
...
def needed_contacts_from_cache(cache: dict | None) -> dict[str, dict]:
    contacts = (cache or {}).get("contacts") or {}
...
def pipeline_row_from_json(place_url: str, info: dict) -> dict:
    ...
    oblast = _s(info.get("bundesland")) or _s(info.get("discovery_bundesland"))
    return {
        ...
        "bundesland": oblast,
        "discovery_bundesland": _s(info.get("discovery_bundesland")),
...
def excel_col_value(rec: dict, col: str) -> str:
    if col == "Obwód":
        return _s(rec.get("Obwód") or rec.get("Oblast") or rec.get("Województwo"))
    return _s(rec.get(col))
...
def find_excel_gaps(contacts: dict[str, dict], export_rows: list[dict]) -> list[dict]:
    ...
    for place_url, info in contacts.items():
        ...
        for col in EXCEL_REQUIRED_IF_JSON_HAS:
            ...
            excel_val = excel_col_value(rec, col)
...
def fill_export_from_json(contacts: dict[str, dict], export_rows: list[dict]) -> tuple[list[dict], int]:
    ...
    for place_url, info in contacts.items():
        ...
        for col, val in filled.items():
            if _s(val) and not excel_col_value(rec, col):
                rec[col] = val
                changed += 1
...
def load_kontakte_rows(xlsx_path: Path) -> list[dict]:
    import pandas as pd  # pyright: ignore[reportMissingImports]

    try:
        df = pd.read_excel(xlsx_path, sheet_name="Kontakte")
    except Exception:
        df = pd.read_excel(xlsx_path)
    records = df.fillna("").to_dict(orient="records")
    out: list[dict] = []
    for rec in records:
        cleaned = {key: _s(val) for key, val in rec.items()}
        out.append(cleaned)
    return out
